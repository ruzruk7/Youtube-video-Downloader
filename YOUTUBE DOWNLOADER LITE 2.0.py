...
# py -m pip install slugify 
import os, pytube, moviepy, moviepy.editor
from pytube.cli import on_progress # enable a progress bar of video download
import slugify # enable error free naming of the video that contain non-alphabet charecters
...
#NOTE UPDATE in cipher.py of pytube package needed editing on line 30: from: var_regex = re.compile(r"^\w+\W")    TO:  var_regex = re.compile(r"^\$*\w+\W") else regex exception raised
...
while active:
    print('------------------------------------------------------MAIN MENU------------------------------------------------------')
    mode = input('Download video, Enter: DOWNLOAD\t\tDifferent Youtube video, Enter: URL\t\t To exit, Enter: QUIT\t\tMultiple videos, Enter: MULTIPLE\t\tAudio Only, Enter: AUDIO\nEntered: ').lower()
    if mode == 'url':
        url = input('\nPlease enter the youtube video\'s URL\nEntered: ')
    if(len(url) > 1):#use_oauth=True, allow_oauth_cache=True
    ...
    if (mode == 'quit' or mode == 'q') or (destination_location == 'quit' or destination_location == 'q'):
        break
    if url == 'quit' or url == 'q':
        break
    if mode == 'audio':
        sec_mode = input('Download single video\'s audio, Enter: SINGLE\t\tMultiple video\'s audio, Enter: MULTIPLE\nEntered: ').lower()
        if sec_mode == 'single':
            filename = slugify.slugify(pytube.YouTube(url).title)
            print(f'Audio file will be named: {filename}')
            print('Retreiving Youtube streams...\n')
            audio = yt.streams.filter(only_audio=True).last()
            print(f'Selected stream is: {audio}')
            print('Beginning download...')
            audio = audio.download(destination_dir,f'{filename}.webm')
            clip = moviepy.editor.AudioFileClip(f'{destination_dir}/{filename}.webm')
            clip.write_audiofile(f'{destination_dir}/{filename}.mp3')
            clip.close()
            print('\n-----------------Audio Download Complete-----------------')
        if sec_mode == 'multiple':
            url_list = [input('Enter the URLs seperated by commas without spaces')]
            for url in url_list:
                ...
                clip.write_audiofile(f'{destination_dir}/{filename}.mp3')
                clip.close()
            print('\n-----------------Multiple Audio Downloads Complete-----------------')
